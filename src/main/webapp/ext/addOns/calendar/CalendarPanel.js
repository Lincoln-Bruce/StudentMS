...
Ext.define('Ext.calendar.CalendarPanel', {
    ...
    // private property
    startDate: Ext.Date.parse(xjs.app.common.common.Util.getBussinessDate(), "Y-m-d"),
    ...
    initComponent: function() {
    	...
    	this.monthText = currentDate.getFullYear() + "年" + currentMonth + "月";
        this.tbar = {
            cls: 'ext-cal-toolbar',
            border: true,
            items: ['->',{
        ...
        if (this.showMonthView || this.viewCount == 0) {
            this.tbar.items.push({
                id: this.id + '-tb-month-btnInnerEl',
                text: this.monthText,
        ...
        if (this.showMonthView) {
            var month = Ext.applyIf({
                ...
                showTodayText: this.showTodayText,
                showTime: this.showTime,
                reinstallData : this.reloadWorkDay,
                listeners: {
                    'weekclick': {
                        fn: function(vw, dt) {
                            this.showWeek(dt);
    ...
    initEventRelay: function(cfg) {
        ...
        cfg.listeners.afterrender = {
            fn: function(c) {
                ...
                this.relayEvents(c, ['eventsrendered', 'eventclick', 'eventover', 'eventout', 'dayclick', 'load',
    ...
    onPrevClick: function() {
    	...
    	var currentMonth = (this.startDate.getMonth() + 1) > 9 ? (this.startDate.getMonth() + 1) : "0" + (this.startDate.getMonth() + 1);
    	this.monthText = this.startDate.getFullYear() + "年" + currentMonth + "月";
        document.getElementById(this.id+"-tb-month-btnInnerEl").innerHTML = this.monthText;
        this.updateNavState();
        this.fireViewChange();
//        this.reloadWorkDay();
    },
